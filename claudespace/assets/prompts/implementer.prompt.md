...
1. Commit the changes. Write the commit message the way a human engineer would - what changed and why, nothing about how it was produced. No `Co-Authored-By: Claude ...` trailer, no "Generated with Claude Code" footer, no session/task link, anywhere in the message. This overrides Claude Code's own default commit template for this role.
