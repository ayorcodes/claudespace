...
**Amendment (implementer, 2026-09-02, per an explicit direct user request during implementation — see the Planning Brief's matching amendment on its Usability NFR).** `cli.py` also accepts `--tmux` (main command and `claudespace watchdog`), resolved the same way `CLAUDESPACE_TERMINAL` is — an override for one invocation, on top of the `config.toml` path above, not a replacement for it. iTerm2 stays the default with zero flags. `--tmux` shipped in `791b1b5`; the pane-exported `CLAUDESPACE_TERMINAL` fix in `ab9ffe8` is downstream of it (a workspace built via the flag alone, with no `config.toml`, otherwise had every later `claudespace-handoff`/`claudespace-msg` invocation — each its own process, with no visibility into the flag — silently resolve back to `ItermBackend`).
