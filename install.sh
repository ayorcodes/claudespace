...
#
# npm is the primary channel (D0): if node/npm are available, or can be
# installed via Homebrew, this installs `@ayorcodes/claudespace` from the
# registry and stops there. Only when node cannot be provided does this fall
# back to the pipx path - the full installer this script used to be,
# retained here as the node-less alternative for Python-native users.
#
...
# Refuse early, before doing anything else: there is no point installing
# node or cloning a repo onto a machine that can never run claudespace.
...
install_via_npm() {
    echo "Installing $NPM_PACKAGE via npm..."
    npm install -g "$NPM_PACKAGE" ||
        die "'npm install -g $NPM_PACKAGE' failed - see output above."
    ...
    if ! zsh -lic 'command -v claudespace >/dev/null 2>&1' >/dev/null 2>&1; then
        ...
        echo "npm's global bin dir may not be on PATH - check with 'npm config get prefix' and add <prefix>/bin to your shell profile." >&2
    fi
...
Install Homebrew (https://brew.sh) or pipx (https://pipx.pypa.io) manually and re-run."
    ...
    # `pipx ensurepath` appends to a shell rc file. If that file does not end
    # in a newline, the appended export is concatenated onto whatever the
    # last line was, destroying both. Make sure every rc file it might touch
    # ends cleanly first. (This has bitten real users - it silently breaks
    # unrelated PATH entries, and the damage is only visible by reading the
    # file.)
    ...
    # Always run this, not just after a fresh pipx install: a pre-existing
    # pipx may never have had `ensurepath` run, which is exactly what leaves
    # PIPX_BIN_DIR off PATH even though this script's own install succeeds.
    #
    # Output is discarded, including stderr: when PATH is already set up
    # pipx prints a yellow "try again with the '--force' flag" advisory
    # there and exits non-zero, which reads like a failure mid-install even
    # though nothing is wrong. The PATH check near the end of this function
    # is what actually verifies the outcome, so this call's own report adds
    # nothing.
    ...
    #     already exists, so a re-install would keep whatever (possibly too
    #     old) interpreter the previous install used.
    #   - `--force` reuses the existing venv, so console scripts that a
    #     newer version no longer declares are left behind in PIPX_BIN_DIR
    #     forever.
    ...
    # running right now. Check a fresh login shell so we report what the
    # user will actually get, rather than guessing.
    if ! zsh -lic 'command -v claudespace >/dev/null 2>&1' >/dev/null 2>&1; then
        echo ""
        echo "warning: 'claudespace' is not on PATH in a new login shell." >&2
...
if BREW="$(find_brew)"; then
    ...
    if "$BREW" install node && has_node; then
        install_via_npm
        exit 0
    fi
    echo "Installing Node.js via Homebrew failed or it's still not on PATH; falling back to the pipx install." >&2
else
    echo "Homebrew not found either; falling back to the pipx install." >&2
fi
